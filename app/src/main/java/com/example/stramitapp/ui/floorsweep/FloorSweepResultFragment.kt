...
class FloorSweepResultFragment : Fragment() {
    ...
    private fun setupObservers() {
        ...
        viewModel.navigateToLocate.observe(viewLifecycleOwner) { pair ->
            pair?.let { (rfid, barcode) ->
                // Example of navigation to a Locate Fragment if it exists
                // findNavController().navigate(R.id.action_nav_floor_sweep_result_to_locate, bundle)
                Toast.makeText(context, "Navigating to Locate: $rfid", Toast.LENGTH_SHORT).show()
